...
This includes reptile, mammal and bird species lists Sheetal shared with us.
...
We first check for how many of the species are `'extinct'` and which one/s. Turns out none of them are.

```{r load data, warning=FALSE}
rawData <- read.csv("results/redlistRead_CIL.csv")
...
```{r reorganize threat categories, warning=FALSE}
...
```{r list species without valid status histories, warning=FALSE}
rawData %>% filter(is.na(year)) %>% select(-code)
```
...
.
```{r assessed before 2010, warning=FALSE}
...
```{r status 2010, message=FALSE, warning=FALSE}
...
```{r status now, message=FALSE, warning=FALSE}
...
```{r status now is from before 2010, warning=FALSE}
print(
  paste(
...
```{r display status order, warning=FALSE}
...
```{r percent change calculation, warning=FALSE}
...
print(
  paste(
    ...
    ": Percentage change in number of endangered species among birds, mammals and reptiles"
  ),
  quote = FALSE
)
```
...
```{r package details, warning=FALSE}
sessionInfo()
```
