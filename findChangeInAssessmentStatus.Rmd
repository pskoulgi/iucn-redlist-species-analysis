...
Use the status history data for each species and calculate the percentage change in number of endangered species from 2010 to present.
...
* We are only interested in changes between "endangered" and non-"endangered" status, but the IUCN Red List web API returns several more categories. Those categories are appropriately collapsed and ordered before proceeding.
* Threat level ordering is according to [IUCN's criteria](https://www.iucnredlist.org/about/faqs#What%20are%20the%20Red%20List%20Categories%20and%20Criteria). Threat level `"vulnerable"` or higher is "endangered".

**NOTE:** But I am unsure whether I am handling `"threatened"` category correctly.
  ...
  mutate(
    cat_reassigned = fct_collapse(
      ...
      fct_relevel("other", "leastConcern", "nearThreatened", "vulnerable",
                      "threatened", "endangered", "criticallyEndangered")
  )
```
...
Percentage change in number of endangered species from 2010 to now can be done over those species that have been assessed in both epochs. Hence, first we find species that have been assessed both before and after 2010.
...
It is the status of the species at the maximum assessment year after 2010 (i.e., most recent year after 2010 and before 2020).

```{r status present, message=FALSE}
...
For periods before 2010 and between 2019-2020, label species as endangered if their assessed threat level is `"vulnerable"` or higher ([see IUCN reference](https://www.iucnredlist.org/about/faqs#What%20are%20the%20Red%20List%20Categories%20and%20Criteria)). Count them and calculate percentage change in total numbers as follows:
...
numEndangeredBefore2010 <- withPreAndPost2010Status %>% 
  mutate(isEndangered = status2010 >= levels(.data$status2010)[5]) %>% 
  filter(isEndangered == TRUE) %>% 
  nrow()
numEndangeredNow <- withPreAndPost2010Status %>% 
  mutate(isEndangered = statusNow >= levels(.data$statusNow)[5]) %>% 
  filter(isEndangered == TRUE) %>% 
  nrow()
