...
* Species which don't appear in the Red List return `NA`, those are dropped 
    ...
    + If synonyms exist but none of them appear in the Red List either, then such species have `synonymousSpeciesUsed = 'all available'`
...
rawData <- read.csv("results/redlistRead_complete_synonymRuns.csv") %>% 
  drop_na(year)
extinctSpecies <- rawData %>% filter(str_detect(str_to_lower(category), 'extinct')) %>% 
  select(sciName)
rawData %>% filter(sciName == extinctSpecies$sciName) %>% 
